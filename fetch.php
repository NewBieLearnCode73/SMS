...
$query = "SELECT students.*, classes.class_name FROM students INNER JOIN classes ON students.id = classes.student_id WHERE students.name LIKE :searchValue OR students.id LIKE :searchValue OR students.email LIKE :searchValue OR students.address LIKE :searchValue OR students.birthdate LIKE :searchValue OR classes.class_name LIKE :searchValue ORDER BY students.id LIMIT :start, :length";
...
$output = array(
    ...
    "recordsFiltered" => get_total_all_records() + 1,
    "data" => $data
);
